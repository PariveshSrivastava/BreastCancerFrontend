...
export default function BreastCancerPredict() {
    ...
    if (file) {
      ...
      try {
        ...
        if (responseData.code === 0) {
          setOutput(responseData.all);
          setPredicted(true);
          calculateAllredScore(responseData.n, responseData.w, responseData.m, responseData.s);
        } else {
          setPredicted(false);
        }
      } catch (error) {
  ...
  const calculateAllredScore = (n, w, m, s) => {
    const prop = (n + w + m) / s;
    let score = 0;
    if (prop === 0) score = 0;
    else if (prop <= 0.01) score = 1;
    else if (prop <= 0.1) score = 2;
    else if (prop <= 0.33) score = 3;
    else if (prop <= 0.66) score = 4;
    else score = 5;

    setAllredScore(score);
  };

  return (
    <div className="bg-gray-100">
      <p className="text-center text-5xl pt-14 text-gray-400">Allred Scoring</p>
      ...
      <div className="flex items-center justify-center p-12">
        <div className="mx-auto w-full max-w-[550px] bg-white rounded-2xl">
          {predicted && response ? (
            <>
              <div className="">
                <div className="flex items-center justify-center w-auto pt-8">
                  <img src={image} alt="input-image-copy" className="h-auto mx-auto" />
                  <img src={`data:image/png;base64,${response.image}`} alt="output" className="h-auto mx-auto" />
                </div>
                <br />
                <div className="flex items-center justify-center">
                  <p className="font-semibold mx-auto">Input</p>
                  <p className="font-semibold mx-auto">Prediction</p>
                </div>
              </div>
              <div className="text-center pt-4">
                <div>
            ...
            <>
              <form className="py-6 px-9" onSubmit={(e) => handleFormSubmit(e)}>
                <div className="pt-4 text-center">
                  <label className="block text-2xl font-semibold text-gray-600 ">Upload File</label>
                  <div className="flex items-center justify-center p-4">
                    <input
